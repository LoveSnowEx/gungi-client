...
enum EventSourceState { CONNECTING, OPEN, CLOSED }  ## The connection is not yet open or reconnecting.  ## The connection is open and ready to communicate.  ## The connection is closed or couldn't be opened.
...
func _check_content_type() -> Error:
	...
	for header: String in response_headers_dict.keys():
		...
		if header.to_lower() == "content-type":
			if value.containsn("text/event-stream"):
				...
				push_error(
					(
						"Received invalid content type from server. Stopping HTTPEventSource. Got '%s'"
						% [value]
					)
				)
	...
	push_error(
		"No Content-Type header found in response from the server. 'Content-Type: text/event-stream' is a requirement. Stopping HTTPEventSource."
	)
	return ERR_INVALID_DATA
